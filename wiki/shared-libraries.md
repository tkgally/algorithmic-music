...
2. **Audible layer** — `synth`, `fx` — **built and validated as of 2026-07-08 (session 018)** in `experiments/lib/synth.js` (FM electric-piano lead, detuned-triangle string/pad, sine+triangle bass — click-safe ramped envelopes) and `experiments/lib/fx.js` (per-voice buses with non-bass high-pass and mono lows, one synthesized-IR convolution reverb, and a saturation → glue-compressor → safety-limiter → trim master chain with the loudness discipline). Both implement the cited topologies in [synthesis-recipes](synthesis-recipes.md) and [effects-and-mixing](effects-and-mixing.md). Validated by an OfflineAudioContext render of the shipping engine (`experiments/tools/render-engine.mjs`): no clipping, no gross discontinuity, no silent gap, background-quiet level across seeds and modes. First consumed by the launch engine ([findings-tonal-classical-engine](findings-tonal-classical-engine.md)).
...
- **`synth` + `fx` audible layer** (`experiments/lib/synth.js`, `experiments/lib/fx.js`, added 2026-07-08, session 018) — sample-free instrument voices (FM keys, detuned-ensemble strings, bass) and the shared master chain (buses, synthesized-IR convolution reverb, glue + limiter), implementing [synthesis-recipes](synthesis-recipes.md) and [effects-and-mixing](effects-and-mixing.md). Browser/offline-only (they take an `AudioContext`), so validated through the render harness rather than the Node suite; first consumed by the launch engine ([findings-tonal-classical-engine](findings-tonal-classical-engine.md)).

`node experiments/tests/run.js` runs 88 passing tests (several containing many internal checks — e.g. the Euclidean-evenness test alone checks all 528 `(k,n)` pairs with 1 ≤ k ≤ n ≤ 32), plus `node experiments/tools/render-measure.mjs` and `node experiments/tools/render-engine.mjs` for the audio harnesses. Things the prototypes settled:
...
The previously-open item — validating the audio layer's *real* timing — was **resolved** by the OfflineAudioContext harness (session 017). As of session 018 the **audible layer (`synth`/`fx`) is also built and validated**, and the whole stack is consumed end to end by the launch **tonal-classical engine** ([findings-tonal-classical-engine](findings-tonal-classical-engine.md)): `theory` + `rng` in the composer, `transport` in the performer/scheduler, `synth` + `fx` in the synthesizer, and `analysis` in the offline render gate. Of the planned modules, only `pattern` and any worklet voices (build-order item 4, optional) remain unbuilt.
