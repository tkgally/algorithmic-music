...
- **Swing = tempo-dependent long–short, not fixed 2:1.** The long:short ratio runs as high as ~3.5:1 at slow tempi and falls toward ~1:1 (straight eighths) by roughly 300 BPM, with the short note holding roughly constant at ~100 ms across medium-to-fast tempi (Friberg & Sundström 2002) — see [jazz-and-improvisation.md](jazz-and-improvisation.md) for the full measured curve and citation (the two pages previously gave different interpolated numbers at the same tempo; this page now defers to that one rather than restating its own). This is a scheduling detail; see [scheduling-and-timing.md](scheduling-and-timing.md).
