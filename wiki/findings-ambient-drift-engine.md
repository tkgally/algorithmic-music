...
**Reverb-tail grain fix (with the engine-01 v0.3 pass).** A listener rated this engine 4/5 but heard "some static-like noise in the middle of some midrange and high notes." Since the ambient voices are pure sines (`bell`, `drone`) and band-limited triangles (`pad`) — none of which alias — the grain could only come from the shared reverb, whose impulse response was decaying *white* noise. Measured directly, a sustained tone's wet-tail short-time-energy coefficient of variation was ≈ 0.49; switching the IR's noise source to **near-Gaussian** (an Irwin-Hall variate, energy-normalized) dropped it to ≈ 0.33 (−31%) without changing brightness or loudness. Details and the control experiment (a third lowpass pole *worsens* grain — so this is not re-darkening) are in [findings-tonal-classical-engine](findings-tonal-classical-engine.md) § v0.3. The engine also now opens its live context with `latencyHint: 'playback'` against real-time underruns.
