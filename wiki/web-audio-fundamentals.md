...
*This project's status (2026-07-11):* the comprehensive site's live engine uses a bare `AudioContext` plus a `setTimeout` lookahead scheduler, so both obstacles apply and live playback stops when an iPhone screen locks. Session 040 documented that; session 041 then built the **pre-render option above as an experimental toggle** — a first-party WAV encoder (`docs/lib/wav.js`) turns an offline render into a `Blob`, an off-screen `<audio>` element plays it (unlocked in-gesture with a looping-silence clip so the async render can swap in), and the Media Session API supplies the lock-screen card and transport controls. It trades the live no-restart setting changes for durability while backgrounded, and its actual screen-locked behavior still needs on-device confirmation. See also [findings-comprehensive-site](findings-comprehensive-site.md) ("Background playback proof-of-concept").
