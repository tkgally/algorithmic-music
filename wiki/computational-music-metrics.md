...
This audio path now has a working realization: the OfflineAudioContext render-and-measure harness (`experiments/tools/render-measure.mjs` + the `analysis` measurement library, [findings-shared-lib-foundation](findings-shared-lib-foundation.md)) — the successor to the `verify.mjs` precedent below, rebuilt around offline rendering and the project's own `analysis` functions (RMS, peak, dBFS, discontinuity/click proxy, amplitude and energy-rise onset detection, silence runs, onset-timing error). It confirmed a first useful result: `transport`-scheduled onsets land sample-accurately in a real render, and it flagged the expected hard case — amplitude/energy onset detection undercounts in dense polyphony, where spectral flux is the fix.
