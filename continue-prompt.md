...
**Phase 2 has begun (2026-07-08): public engines live in `docs/`.** Phase 1 built a mature, reviewed knowledge base and a validated first-party library foundation (`rng`, `transport`, `theory`, `analysis`, `synth`, `fx`) plus a worked composition algorithm; five engines (tonal-classical, ambient-drift, groove-lofi, cantabile, percussion) are shipped under `docs/engines/`, cataloged in the hub `docs/index.html`. Build new engines from `wiki/project-roadmap.md` Phase 2 + `wiki/engine-architecture.md`: self-contained, **numbered** folders under `docs/engines/` that **vendor** (copy in) the shared libraries from `experiments/lib/`, added to the hub. Keep maintaining the wiki alongside — findings from engine work still land there.
