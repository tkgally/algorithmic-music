...
```
node experiments/tests/run.js      # 25 assertions across rng + transport
```
...
`lib/rng.js` and `lib/transport.js` are written as **dual-format (UMD-lite)**
files: the same source loads as CommonJS in Node (`require('./rng.js')`) and as
a browser global via a classic `<script src>` (`window.AM.rng`, `window.AM.transport`).
...
("A Tale of Two Clocks", 2013). Ideas and algorithms are borrowed and attributed;
