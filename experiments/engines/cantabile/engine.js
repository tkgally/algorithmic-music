...
;(function (global, factory) {
  ...
  function renderPlan(opts) {
    ...
    // previous note per role (for portamento + interval direction)
    const prev = {};
    ...
    for (const n of ordered) {
      ...
      // --- articulation (legato for expressive voices; ring the goals) ---
      ...
      if (role === 'lead' || role === 'counter' || role === 'inner') {
        ...
        else if (n.tags.includes('goal') || n.durBeats >= 2) artic = 1.0;
        else artic = 0.96;                                   // slight legato overlap
      } else if (role === 'comp') { artic = n.tags.includes('arp') ? 1.0 : 0.96; }
      else artic = 0.9;                                      // bass
      const durSec = Math.max(0.08, slot * artic);
      ...
      if (prof) {
        ...
        if (p && Math.abs(p.midi - n.midi) > 0 && Math.abs(p.midi - n.midi) <= 5 && n.beat - (p.beat + p.durBeats) < 0.51 && !n.tags.includes('goal')) {
        ...
        expr = {
          ...
          bright, grit, grain: clamp(brDefault, 0, 1),
