...
| `docs/` | **Public site via GitHub Pages.** Entry point `docs/index.html` = the Phase 3 comprehensive site (an early landing page for now). The five **preliminary test engines** + their hub live in `docs/preliminary-tests/` (engines under `docs/preliminary-tests/engines/`, hub `docs/preliminary-tests/index.html`). Documents prepared for Tom's review also go here. | Per `wiki/engine-architecture.md`; engines vendor the shared libs; temporary review docs may be deleted once superseded |

## Hard rules

1. **Phase 2 shipped five public engines; Phase 3 — the comprehensive site — is now the primary track (Tom, 2026-07-09).** The five engines are self-contained, **numbered** folders that **vendor** (copy in) the shared libraries from `experiments/lib/`. Per Tom's 2026-07-09 decision they now live under **`docs/preliminary-tests/`** (engines in `docs/preliminary-tests/engines/`, hub `docs/preliminary-tests/index.html`), reframed as *preliminary test engines* — style studies and validated component sources. The public entry point **`docs/index.html` is now the comprehensive site** (an early landing page for now, linking to the preliminary tests). **The long-term goal (Tom, 2026-07-09):** one comprehensive self-contained site composing instrumental music in many pre-established *and* on-the-fly original styles, three control tiers (Start/Intermediate/Advanced), compact seeded URLs, original annotated shared libraries, all in one folder — read `wiki/comprehensive-site-vision.md` and roadmap Phase 3. Design docs (control taxonomy → style-pack schema → site architecture) come before site code and are **placed in `docs/` for Tom's async review** (durable decisions still filed in the wiki). The knowledge base is maintained alongside throughout; build engines/site per `wiki/engine-architecture.md`.
