...
1. **Phase 2 has begun (Tom's explicit request, 2026-07-08): public engines now live in `docs/`.** Engine 01 (`docs/engines/01-tonal-classical/`) is the launch engine. New engines are self-contained, **numbered** folders that **vendor** (copy in) the shared libraries from `experiments/lib/`, and are added to the hub `docs/index.html`. Build from `wiki/project-roadmap.md` Phase 2 + `wiki/engine-architecture.md`. (The knowledge base continues to be maintained alongside.) **The long-term goal was defined by Tom on 2026-07-09** — one comprehensive self-contained site composing instrumental music in many pre-established *and* on-the-fly original styles, three control tiers, compact seeded URLs, original annotated shared libraries, all in one folder: read `wiki/comprehensive-site-vision.md` and roadmap Phase 3. Design docs (control taxonomy, style-pack schema, site architecture) come before site code, reviewed by Tom asynchronously.
