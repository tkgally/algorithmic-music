...
| `docs/` | **Public site via GitHub Pages.** Entry point `docs/index.html` = **the comprehensive site — built session 034; given its "Console" dark-instrument-panel visual redesign + a standalone `docs/about.html` (operating instructions live there) in sessions 044–045; live at v1.0.0** — with its libraries in `docs/lib/`, genre packs in `docs/styles/`, app in `docs/app.js`, styles in `docs/css/site.css`, self-hosted fonts in `docs/fonts/`, dev self-test `docs/_selftest.html`. The five **preliminary test engines** + their hub live in `docs/preliminary-tests/` (engines under `docs/preliminary-tests/engines/`, hub `docs/preliminary-tests/index.html`) — regression baselines, never modified. Design docs + documents prepared for Tom's review live in `docs/design/` / `docs/`. | Per `wiki/engine-architecture.md` + `docs/design/site-architecture.html`; engines vendor the shared libs; temporary review docs may be deleted once superseded |

## Hard rules

1. **The comprehensive site is BUILT (session 034, Phase 3c) and is the public entry point at `docs/index.html`; Phase 3d — the listen → tune → deepen improvement loop — is now the primary track.** The site realizes Tom's 2026-07-09 goal (read `wiki/comprehensive-site-vision.md`): instrumental music in pre-established *and* on-the-fly original styles, composed **just-in-time** (live no-restart setting changes; a two-bar crossfade on genre swaps), two-genre melds, three control tiers (Start/Intermediate/Advanced), compact seeded URLs, all-original annotated libraries — one folder: `docs/index.html` + `docs/lib/` + `docs/styles/` + `docs/app.js` + `docs/_selftest.html` (+ `docs/css/`). *(Since session 039 the picker shows 7 of the 8 built genres — Jazz is hidden per Tom, 2026-07-11, but its pack stays registered so old links/melds play; `HIDDEN_STYLES` in `docs/app.js`.)* Its visual layer is the **"Console"** redesign (dark instrument-panel — `docs/css/site.css`, self-hosted `docs/fonts/`, standalone `docs/about.html` holding the operating instructions), shipped at **v1.0.0** (sessions 044–045); bump `SITE_VERSION` in `docs/app.js` on user-visible changes (see Conventions). Its design contract is the three Phase-3b docs in `docs/design/`; its build findings are `wiki/findings-comprehensive-site.md`; its gates (`site-check.js`, `render-site.mjs`, `site-smoke.mjs` in `experiments/tools/`) must stay green. The five Phase-2 engines live under **`docs/preliminary-tests/`** (hub + numbered folders that vendor the shared libs from `experiments/lib/`) as *preliminary test engines* — style studies, component sources, and **regression baselines: never modify them**. Review documents for Tom still go in `docs/` (durable decisions filed in the wiki). The knowledge base is maintained alongside throughout; site work follows `wiki/engine-architecture.md` + the design docs.
...
- Engine architecture: `wiki/engine-architecture.md`.
- **Review docs → `docs/`** (Tom, 2026-07-09): documents/pages prepared for Tom's asynchronous review go in `docs/` so he can read them via GitHub Pages; temporary ones may be deleted once superseded. Durable decisions from any review doc are still filed in the wiki (rule 4) so they survive cleanup.
...
- **Site version:** `SITE_VERSION` in `docs/app.js` (shown top-right, currently **v1.0.0**). Bump it on user-visible site changes and note the bump in the session log — **patch** (x.y.**Z**) for small tweaks/fixes, **minor** (x.**Y**.0) for new controls/features, **major** (**X**.0.0) for overhauls like a visual redesign.
