 ...
 *
 * SITE COPY (docs/lib, comprehensive site) — two additive extensions over the
 * engines' vendored copies (which stay byte-frozen in docs/preliminary-tests/):
 *   - opts.out — the node the chain terminates into (default ctx.destination).
 *     The site's live genre/blend swap runs TWO whole chains side by side and
 *     equal-power crossfades between them over two bars (site-architecture §6),
 *     so each chain must terminate into its own fade gain rather than hard-wire
 *     ctx.destination.
 *   - setWidth(w) — a mid/side stereo-width macro (0 = mono, 1 = as-mixed,
 *     up to 1.4 gently wider), the control-taxonomy "Stereo width" control,
 *     implemented as an M/S matrix (split -> mid/side sums -> re-matrix) so it
 *     is mono-safe: the mid band never moves, w scales only the side band.
 */
;(function (global, factory) {
  ...
  function createMasterChain(ctx, opts) {
    ...
    sum.connect(sat); sat.connect(glue); glue.connect(limiter); limiter.connect(tone); tone.connect(master);

    // Mid/side stereo-width stage: L/R -> M=(L+R)/2, S=(L-R)/2 -> back to
    // L=M+wS, R=M-wS. With w=1 this is exactly identity; setWidth scales only
    // the side gains, so lows and the mono core are untouched (mono-safe).
    const widthIn = ctx.createChannelSplitter(2);
    const midSum = ctx.createGain(); midSum.gain.value = 0.5;
    const sideSum = ctx.createGain(); sideSum.gain.value = 0.5;
    const sideInvIn = ctx.createGain(); sideInvIn.gain.value = -1;
    const sidePos = ctx.createGain(); sidePos.gain.value = 1;   // width w rides here…
    const sideNeg = ctx.createGain(); sideNeg.gain.value = -1;  // …and here (as -w)
    const widthOut = ctx.createChannelMerger(2);
    master.connect(widthIn);
    widthIn.connect(midSum, 0); widthIn.connect(midSum, 1);           // M = (L+R)/2
    widthIn.connect(sideSum, 0); widthIn.connect(sideInvIn, 1); sideInvIn.connect(sideSum); // S = (L-R)/2
    midSum.connect(widthOut, 0, 0); midSum.connect(widthOut, 0, 1);   // M into both
    sideSum.connect(sidePos); sidePos.connect(widthOut, 0, 0);        // +wS into L
    sideSum.connect(sideNeg); sideNeg.connect(widthOut, 0, 1);        // -wS into R
    widthOut.connect(opts.out || ctx.destination);
    ...
    return {
      ...
      /** Stereo width, mid/side: 0 = mono, 1 = as-mixed, ≤1.4 gently wider. */
      setWidth(w) {
        const s = Math.max(0, Math.min(1.4, w));
        sidePos.gain.setTargetAtTime(s, ctx.currentTime, 0.05);
        sideNeg.gain.setTargetAtTime(-s, ctx.currentTime, 0.05);
      },
    };
  }
